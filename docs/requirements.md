...
5. There is **no centralized server storing party data**. A stateless WebSocket relay routes messages between players; nothing about the party is persisted server-side. The relay is stateless between reconnects — closing the last member's connection evicts the in-memory party.
