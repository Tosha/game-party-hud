...
| — | **Phase 9 (optional, future)** | **Self-hosted build of the same relay — see below** |
| 22 | Extract platform-agnostic Room logic | Factor `PartyRoom` into a plain class + per-platform adapters |
| 23 | Node.js `ws`-based server | `npm run start:local` spins up the same protocol on bare Node |
| 24 | Node test suite | Vitest reuses the existing fixtures against the Node adapter |
| 25 | OS service files | Windows Task Scheduler script + systemd unit + run instructions |
| 26 | Public exposure | Cloudflare Tunnel recipe (free, stable URL, no router config) |
| 27 | Operator runbook | `relay/SELFHOST.md` — start/stop/upgrade/diagnose |
...
## Phase 9 — Self-hosted build of the same relay (optional, future)

**Why this phase exists at all:** Cloudflare's terms, pricing, or availability could change, or you might prefer running party infrastructure on a home PC you own. This phase lets the same `relay/` project produce a second build artefact — a plain Node.js process that speaks the exact same wire protocol — so the client can switch from the Worker URL to a self-hosted URL by editing one config field and requires **zero** client-code changes.

**Do not execute before Phase 0–8 are merged and working.** This is additive. The CF Worker deploy keeps working after these tasks land.

**Sharing code:** the room-state logic (roster, broadcast, caps) is platform-agnostic. The only CF-specific code is the `WebSocketPair` / `DurableObjectState` wrapping and the request routing. Task 22 extracts the former into a reusable `RoomCore` module; Tasks 23–24 bolt on a Node adapter.

### Task 22: Extract platform-agnostic Room logic

**Files:**
- Create: `relay/src/room-core.ts`
- Modify: `relay/src/room.ts` (CF Worker adapter now thin wrapper)
- Modify: `relay/test/room.test.ts` (imports unchanged; regression check only)

- [ ] **Step 1: Write the module contract with failing tests**

File: `relay/test/room-core.test.ts`
```typescript
import { describe, expect, it, vi } from "vitest";
import { RoomCore, type Connection } from "../src/room-core";

function fakeConnection(): Connection & { sent: string[] } {
  const sent: string[] = [];
  return {
...
describe("RoomCore", () => {
  it("responds to join with a welcome listing current members", () => {
    const room = new RoomCore();
    const a = fakeConnection();
    room.onMessage(a, '{"type":"join","peerId":"peer-a"}');
    expect(a.sent).toEqual(['{"type":"welcome","peerId":"peer-a","members":[]}']);

    const b = fakeConnection();
    room.onMessage(b, '{"type":"join","peerId":"peer-b"}');
    expect(b.sent).toEqual(['{"type":"welcome","peerId":"peer-b","members":["peer-a"]}']);
    expect(a.sent[1]).toBe('{"type":"peer-joined","peerId":"peer-b"}');
  });

  it("relays broadcasts to other members", () => {
    const room = new RoomCore();
    const a = fakeConnection();
    const b = fakeConnection();
    room.onMessage(a, '{"type":"join","peerId":"peer-a"}');
    ...
    room.onMessage(a, '{"type":"broadcast","payload":"hello"}');
    expect(a.sent).toEqual([]);
    expect(b.sent).toEqual(['{"type":"message","fromPeerId":"peer-a","payload":"hello"}']);
  });

  it("broadcasts peer-left on disconnect", () => {
    const room = new RoomCore();
    const a = fakeConnection();
    const b = fakeConnection();
    room.onMessage(a, '{"type":"join","peerId":"peer-a"}');
    ...
    room.onDisconnect(b);
    expect(a.sent).toEqual(['{"type":"peer-left","peerId":"peer-b"}']);
  });

  it("rejects a 26th peer with party-full then invokes close", () => {
    const room = new RoomCore();
    for (let i = 0; i < 25; i++) {
      room.onMessage(fakeConnection(), `{"type":"join","peerId":"peer-${i}"}`);
    }
    const overflow = fakeConnection();
    room.onMessage(overflow, '{"type":"join","peerId":"peer-26"}');
    expect(overflow.sent).toEqual(['{"type":"error","reason":"party-full"}']);
    expect(overflow.close).toHaveBeenCalledOnce();
  });

  it("rejects a frame larger than 4 KiB", () => {
    const room = new RoomCore();
    const a = fakeConnection();
    room.onMessage(a, '{"type":"join","peerId":"peer-a"}');
    a.sent.length = 0;
    const big = 'x'.repeat(5000);
    room.onMessage(a, `{"type":"broadcast","payload":"${big}"}`);
    expect(a.sent).toEqual(['{"type":"error","reason":"message-too-large"}']);
  });
});
```
...
```bash
cd relay
npm test -- --run room-core
```

Expected: "Cannot find module '../src/room-core'".

- [ ] **Step 3: Extract the room-state machinery**

File: `relay/src/room-core.ts`
```typescript
import {
  type ServerMessage,
  decodeClientMessage,
...
/**
 * Platform-agnostic WebSocket abstraction. Both the Cloudflare Worker DO
 * adapter and the Node ws adapter provide one of these per connected peer.
 */
...
/**
 * Pure protocol handler. Holds per-party state, has no knowledge of the
 * underlying transport. The adapters in room.ts (Cloudflare) and
 * node-server.ts (Node) call onMessage / onDisconnect on behalf of their
 * sockets and implement Connection.
 */
export class RoomCore {
  private connections = new Map<Connection, string>(); // socket -> peerId, once joined
  private members = new Map<string, Member>();

  onMessage(conn: Connection, raw: string): void {
    if (raw.length > MAX_FRAME_BYTES) {
      this.send(conn, { type: "error", reason: "message-too-large" });
      return;
    }
    const msg = decodeClientMessage(raw);
    if (!msg) return;
    ...
    if (msg.type === "join") {
      if (this.connections.has(conn)) return; // ignore duplicate join
      if (this.members.size >= MAX_PEERS) {
        this.send(conn, { type: "error", reason: "party-full" });
        conn.close(1008, "party-full");
        return;
      }
      if (this.members.has(msg.peerId)) {
        this.send(conn, { type: "error", reason: "duplicate-peer" });
        conn.close(1008, "duplicate-peer");
        return;
      }
      const existing = [...this.members.keys()];
      this.connections.set(conn, msg.peerId);
      this.members.set(msg.peerId, { connection: conn, peerId: msg.peerId });
      this.send(conn, { type: "welcome", peerId: msg.peerId, members: existing });
      this.broadcastExcept(msg.peerId, { type: "peer-joined", peerId: msg.peerId });
      return;
    }

    if (msg.type === "broadcast") {
      const peerId = this.connections.get(conn);
      if (!peerId) return; // ignore pre-join
      this.broadcastExcept(peerId, {
        type: "message",
        fromPeerId: peerId,
        payload: msg.payload,
  ...
  private broadcastExcept(excludePeerId: string, msg: ServerMessage): void {
    ...
    for (const m of this.members.values()) {
      if (m.peerId === excludePeerId) continue;
      try { m.connection.send(encoded); } catch { /* dead; disconnect handler cleans up */ }
    }
  }
}
```

- [ ] **Step 4: Rewrite `relay/src/room.ts` to be a thin Cloudflare adapter over `RoomCore`**

Replace the entire contents of `relay/src/room.ts`:

```typescript
import { type Connection, RoomCore } from "./room-core";

/**
 * Cloudflare Durable Object adapter. One instance per party id (routed by
 * worker.ts via env.PARTY_ROOM.idFromName(partyId)). Wraps the ws pair as a
 * Connection and delegates all protocol logic to RoomCore.
 */
export class PartyRoom {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    server.addEventListener("message", (ev) => {
      const raw = typeof ev.data === "string" ? ev.data : "";
      this.core.onMessage(conn, raw);
    });
    server.addEventListener("close", () => this.core.onDisconnect(conn));
    server.addEventListener("error", () => this.core.onDisconnect(conn));
...
Expected: all pre-existing room.test.ts tests still pass (behaviour unchanged), plus the 5 new room-core.test.ts tests pass.

- [ ] **Step 6: Commit**

```bash
git add relay/src/room-core.ts relay/src/room.ts relay/test/room-core.test.ts
git commit -m "refactor(relay): extract RoomCore — platform-agnostic room state

CF Worker DO (PartyRoom) is now a ~30-line adapter over RoomCore. Opens
the door for a Node server adapter that reuses every protocol test."
```

### Task 23: Node.js `ws`-based server adapter

**Files:**
- Modify: `relay/package.json` (add `ws`, `@types/ws`, `tsx`; new `start:local` script)
- Create: `relay/src/node-server.ts`
- Create: `relay/tsconfig.node.json`

- [ ] **Step 1: Add dependencies**

Edit `relay/package.json` — add to `devDependencies`:
...
```json
    "start:local": "tsx src/node-server.ts",
    "build:local": "tsc -p tsconfig.node.json"
```

- [ ] **Step 2: Create `relay/tsconfig.node.json`**

File: `relay/tsconfig.node.json`
```json
{
  ...
  "include": ["src/node-server.ts", "src/room-core.ts", "src/protocol.ts"]
}
```

- [ ] **Step 3: Write the Node server**

File: `relay/src/node-server.ts`
```typescript
...
// One RoomCore per party id. Map entries evict automatically when the last
// member disconnects (see trackRoom below).
const rooms = new Map<string, RoomCore>();

function getOrCreateRoom(partyId: string): RoomCore {
  let room = rooms.get(partyId);
  if (!room) {
    room = new RoomCore();
...
function parsePartyId(req: IncomingMessage): string | null {
  if (!req.url) return null;
  const match = /^\/party\/([A-Za-z0-9_-]{1,32})$/.exec(req.url);
...
http.on("upgrade", (req, socket, head) => {
  const partyId = parsePartyId(req);
  if (!partyId) {
    socket.write("HTTP/1.1 404 Not Found\r\n\r\n");
    socket.destroy();
    return;
  }
  wss.handleUpgrade(req, socket, head, (ws) => attach(ws, partyId));
});

function attach(ws: WebSocket, partyId: string): void {
  ...
  ws.on("message", (data) => {
    ...
    room.onMessage(conn, raw);
  });
  ws.on("close", () => trackRoomExit(partyId, room, conn));
  ws.on("error", () => trackRoomExit(partyId, room, conn));
}

function trackRoomExit(partyId: string, room: RoomCore, conn: Connection): void {
  room.onDisconnect(conn);
  if (room.peerCount === 0) rooms.delete(partyId);
}

http.listen(PORT, HOST, () => {
  // eslint-disable-next-line no-console
  console.log(`gph-relay (self-hosted) listening on ws://${HOST}:${PORT}/party/<id>`);
});
```
...
Expected: zero errors. `dist-node/` appears with compiled `node-server.js`.

- [ ] **Step 5: Manual smoke-test the Node server**

Terminal 1:
```bash
npm run start:local
# Expect: gph-relay (self-hosted) listening on ws://0.0.0.0:8787/party/<id>
```
...
wscat -c ws://localhost:8787/party/LOCAL
> {"type":"join","peerId":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}
< {"type":"welcome","peerId":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","members":[]}
```

`curl .../health` should now report `rooms=1`.

- [ ] **Step 6: Commit**

```bash
git add relay/package.json relay/package-lock.json relay/tsconfig.node.json relay/src/node-server.ts
git commit -m "feat(relay): Node.js self-hosted adapter atop RoomCore

Same wire protocol, same caps, same tests. Run with npm run start:local;
health check at GET /health; WS at /party/<id>."
```

### Task 24: Node test suite (reuse existing fixtures)

**Files:**
- Create: `relay/test/node-server.test.ts`

- [ ] **Step 1: Write the failing test**

File: `relay/test/node-server.test.ts`
```typescript
...
const PORT = 18788; // not 8787 to avoid collision with a running dev server
let server: ChildProcess;

beforeAll(async () => {
  server = spawn("npx", ["tsx", "src/node-server.ts"], {
    env: { ...process.env, PORT: String(PORT), HOST: "127.0.0.1" },
    stdio: ["ignore", "pipe", "pipe"],
  });
  // Wait for the "listening on" line or 3 s.
  await new Promise<void>((resolve, reject) => {
    const t = setTimeout(() => reject(new Error("server start timeout")), 3000);
    server.stdout!.on("data", (d) => {
      if (d.toString().includes("listening on")) { clearTimeout(t); resolve(); }
    });
  });
});
...
function connect(partyId: string): Promise<WebSocket> {
  return new Promise((resolve, reject) => {
    const ws = new WebSocket(`ws://127.0.0.1:${PORT}/party/${partyId}`);
...
function nextMessage(ws: WebSocket): Promise<string> {
  return new Promise((resolve) => ws.once("message", (d) => resolve(d.toString("utf8"))));
}

describe("Node server (integration)", () => {
  it("welcomes a fresh joiner", async () => {
    const ws = await connect("NODE1");
    ws.send(fixtures.join);
    const welcome = await nextMessage(ws);
    expect(JSON.parse(welcome)).toEqual({
      type: "welcome",
      peerId: "a5bdd9f976fe4da6a5dc11035522d1ddbeefcafe",
      members: [],
    });
  ...
  it("relays a broadcast from A to B", async () => {
    const a = await connect("NODE2");
    a.send('{"type":"join","peerId":"node-peer-a"}');
    await nextMessage(a); // welcome

    const b = await connect("NODE2");
    b.send('{"type":"join","peerId":"node-peer-b"}');
    await nextMessage(b); // welcome
    await nextMessage(a); // peer-joined

    b.send('{"type":"broadcast","payload":"hi from b"}');
    expect(JSON.parse(await nextMessage(a))).toEqual({
      type: "message",
      fromPeerId: "node-peer-b",
      payload: "hi from b",
    });
    a.close(); b.close();
  });
...
- [ ] **Step 2: Exclude this test from the default Vitest run (it spawns processes; keep it opt-in)**

Edit `relay/vitest.config.ts`:
...
export default defineWorkersConfig({
  test: {
    ...
    // node-server.test runs against a spawned Node process; not compatible
    // with the Miniflare pool. Run it explicitly with `npm run test:node`.
    exclude: ["**/node-server.test.ts", "**/node_modules/**"],
  },
});
```

Add a script to `relay/package.json`:

```json
    "test:node": "vitest run test/node-server.test.ts --pool=threads --poolOptions.workers=undefined"
```

Actually that's fragile; use a simpler configuration:

Replace the `test:node` script line with:

```json
    "test:node": "vitest run --config vitest.node.config.ts"
```

And create `relay/vitest.node.config.ts`:
...
export default defineConfig({
  test: {
    include: ["test/node-server.test.ts"],
    testTimeout: 15_000,
  },
});
```
...
Expected: both tests pass. If the spawn fails, confirm `npx tsx` resolves (`npm install` must have completed); on Windows, `tsx` is `node_modules\.bin\tsx.cmd`.

- [ ] **Step 4: Commit**

```bash
git add relay/test/node-server.test.ts relay/vitest.config.ts relay/vitest.node.config.ts relay/package.json
git commit -m "test(relay): Node server integration tests against spawned server"
```

### Task 25: OS service files — start on boot

**Files:**
- Create: `relay/scripts/gph-relay.service` (systemd unit for Linux)
- Create: `relay/scripts/install-windows-task.ps1` (Windows Task Scheduler)
- Create: `relay/scripts/uninstall-windows-task.ps1`

- [ ] **Step 1: Create `relay/scripts/gph-relay.service`**

File: `relay/scripts/gph-relay.service`
```ini
# Systemd unit for the self-hosted gph-relay Node server. Install:
#   sudo cp relay/scripts/gph-relay.service /etc/systemd/system/
#   sudo systemctl daemon-reload
#   sudo systemctl enable --now gph-relay
# Ops:
#   sudo journalctl -u gph-relay -f     # tail logs
#   sudo systemctl restart gph-relay    # rolling restart

[Unit]
Description=Game Party HUD relay
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
# Adjust User + WorkingDirectory to match where you cloned the repo.
User=%u
WorkingDirectory=%h/game-party-hud/relay
ExecStart=/usr/bin/env npx tsx src/node-server.ts
Restart=on-failure
RestartSec=5
Environment=PORT=8787
Environment=HOST=127.0.0.1
# Recommend exposing via Cloudflare Tunnel (Task 26) rather than binding to 0.0.0.0.

[Install]
WantedBy=multi-user.target
```

- [ ] **Step 2: Create Windows Task Scheduler scripts**

File: `relay/scripts/install-windows-task.ps1`
```powershell
# Registers a scheduled task that runs the relay on user logon, restarting on
# failure. Run once, as the user whose account will own the relay process:
#   powershell -ExecutionPolicy Bypass -File relay\scripts\install-windows-task.ps1
#
# After install: the task is visible in Task Scheduler -> "Game Party HUD Relay".
# To see logs: attach PowerShell to the process or redirect stdout in the Action.
...
Register-ScheduledTask -TaskName "Game Party HUD Relay" `
    -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Force

Write-Host "Installed. Start now with: Start-ScheduledTask -TaskName 'Game Party HUD Relay'"
```

File: `relay/scripts/uninstall-windows-task.ps1`
```powershell
$ErrorActionPreference = "Stop"
Unregister-ScheduledTask -TaskName "Game Party HUD Relay" -Confirm:$false
...
```bash
git add relay/scripts/
git commit -m "feat(relay): OS service templates — systemd and Windows Task Scheduler"
```

### Task 26: Public exposure — Cloudflare Tunnel

The Node server only listens on `127.0.0.1:8787` in the default config. To reach it from another player's machine, you need either (a) a port-forward + DDNS, which requires router access and a public IP; or (b) a tunnel service. Cloudflare Tunnel is free, stable, and the common choice.

**Files:**
- Modify: `relay/README.md` (add a Self-Host section)
- Create: `relay/SELFHOST.md` (full operator runbook — lands in Task 27)

- [ ] **Step 1: Install `cloudflared`**

- Windows: download `cloudflared-windows-amd64.exe` from Cloudflare's releases page, rename to `cloudflared.exe`, put on PATH.
- Linux (Debian/Ubuntu): `sudo apt install cloudflared` (repo at https://pkg.cloudflare.com/) or download the `.deb` from releases.
- Alternatively, use the statically linked binary from the Cloudflare Tunnel download page.

- [ ] **Step 2: Authenticate and create a named tunnel**

```bash
cloudflared tunnel login               # Opens browser, pick your domain or choose "trycloudflare.com"
cloudflared tunnel create gph-relay    # Creates a tunnel; prints the tunnel UUID
```

The command writes a credentials file (typical path: `~/.cloudflared/<UUID>.json`).

- [ ] **Step 3: Create a tunnel config file**

File: `~/.cloudflared/config.yml` (Linux/macOS) or `%USERPROFILE%\.cloudflared\config.yml` (Windows)
```yaml
tunnel: gph-relay
credentials-file: /absolute/path/to/<UUID>.json

ingress:
  - hostname: gph-relay.your-domain.example
    service: http://127.0.0.1:8787
  - service: http_status:404
```

If you don't own a domain, use a Quick Tunnel instead — skip the named tunnel and just run:

```bash
cloudflared tunnel --url http://127.0.0.1:8787
```

Cloudflared prints a generated `https://<random>.trycloudflare.com` URL; that's ephemeral (it changes every restart) but free and zero-config.

- [ ] **Step 4: Route the domain (named tunnel only)**

```bash
cloudflared tunnel route dns gph-relay gph-relay.your-domain.example
```

This creates a CNAME in your Cloudflare DNS pointing the hostname at the tunnel. Only needed for named tunnels.

- [ ] **Step 5: Run the tunnel alongside the relay**

Two terminals (or two services):

```bash
# Terminal 1
cd relay && npm run start:local

# Terminal 2
cloudflared tunnel run gph-relay
```

For production, register both under systemd / Task Scheduler (Task 25 handles the relay; add a second unit for `cloudflared`).

- [ ] **Step 6: Update `config.json` on each client**

Each player edits their `%AppData%\GamePartyHud\config.json` to:

```json
{
  "RelayUrl": "wss://gph-relay.your-domain.example"
}
```

(Or the Quick Tunnel URL.) Restart the app.

- [ ] **Step 7: End-to-end smoke with two peers via the tunnel**

Same checklist as Phase 8 Task 21, but `RelayUrl` pointed at the tunnelled self-hosted server. Expected: identical behaviour, two peers see each other.

- [ ] **Step 8: Commit**

No code changes; this task is an operator runbook only. The runbook contents land in Task 27.

### Task 27: Operator runbook — `relay/SELFHOST.md`

**Files:**
- Create: `relay/SELFHOST.md`

- [ ] **Step 1: Write the runbook**

File: `relay/SELFHOST.md`
````markdown
# Self-hosted gph-relay — operator runbook

This page documents running the relay as a plain Node.js process on a machine
you own, exposed via a Cloudflare Tunnel. The wire protocol is identical to the
Cloudflare Worker build in the same folder, so you can migrate clients from one
to the other by changing the `RelayUrl` field in each user's `config.json`.

Use this when:
- You don't want a Cloudflare account.
- Your home network can run a small process 24/7.
- You want full control over the code that routes your party's traffic.

Use the Cloudflare Worker build (the one `README.md` describes) when:
- You don't have an always-on machine.
- You'd rather pay Cloudflare $0–5 / month to handle uptime.

## Prerequisites

- Node.js ≥ 20 (`node --version`)
- A Cloudflare account for the tunnel (free; no credit card needed)
- `cloudflared` binary on PATH
- Port `8787/tcp` free on `127.0.0.1` (change via `PORT` env var)

## First-time setup

```bash
git clone https://github.com/<you>/game-party-hud.git
cd game-party-hud/relay
npm install
```
...
- Linux: see `scripts/gph-relay.service`.
- Windows: see `scripts/install-windows-task.ps1`.
...
Named tunnel (stable URL under your domain):

```bash
cloudflared tunnel login
cloudflared tunnel create gph-relay
cloudflared tunnel route dns gph-relay gph-relay.<your-domain>
cloudflared tunnel run gph-relay
```

## Client config

Each player's `%AppData%\GamePartyHud\config.json` (Windows — adjust for other
OSes) sets `RelayUrl` to the tunnel URL, e.g.

```json
{ "RelayUrl": "wss://gph-relay.<your-domain>" }
```
...
- Relay logs go to stdout. Under systemd: `journalctl -u gph-relay -f`. Under
  Task Scheduler: stdout is captured if you redirect it in the action.
- Health check: `curl http://127.0.0.1:8787/health` → `ok rooms=N`.
- Tunnel logs: `cloudflared tunnel run gph-relay --loglevel debug`.
- If clients see `RelayClient: ✗ could not connect to wss://...`, check in order:
  1. Node process running? (`curl` health endpoint)
  2. `cloudflared` running? (logs last line)
  3. DNS propagation? (`dig gph-relay.<your-domain>`)
  4. Cloudflare dashboard → Zero Trust → Tunnels — tunnel shows Healthy.

## Migration from Cloudflare Worker to self-hosted (or back)

1. Spin up the new relay (Node + tunnel, OR `wrangler deploy`).
2. Update `config.json` on every client to the new `RelayUrl`.
3. Everyone rejoins their party — parties don't persist across the switch; the
   6-char code is just a room name, so rejoining is instant.

There is **no** server-side migration — both relays are stateless.

## Cost

Zero. The Node process uses <100 MB RAM idle and ~0 % CPU. The tunnel is free.
If you worry about your ISP's policies on hosted services, check their ToS;
most residential plans allow this kind of low-traffic outbound use.
````

- [ ] **Step 2: Commit**

```bash
git add relay/SELFHOST.md
git commit -m "docs(relay): operator runbook for self-hosted Node relay via CF Tunnel"
```

---

## Appendix A — Hosting options comparison

Pricing and limits are summarised as of late 2024 / early 2025; confirm current
numbers on each vendor's pricing page before committing. The ranking below is
the important part, and it's stable.

### The matrix

| Host | Free tier adequate? | $/mo at forecast usage | WebSockets | Cold-start | Ops burden | Vendor lock-in |
|---|---|---|---|---|---|---|
| **Cloudflare Workers + Durable Objects** | Yes — Workers Paid $5/mo flat covers it | **$0–5** | Native (hibernate API available) | None on DOs | Zero | Medium (DO API) |
| **PartyKit** | Free hobby; paid tiers above | $0 (hobby) / $20+ (prod) | Native, purpose-built | None | Zero | High (PartyKit SDK) |
| **Fly.io** | Limited (3 small shared VMs) | ~$2/mo shared-cpu-1x 256MB | Standard Node, no extras | None (persistent VM) | Small (Dockerfile, logs) | Low (plain Docker) |
| **Render.com** | 750 hrs/mo, **spins down after 15 min idle** | $7/mo Starter to avoid spin-down | Standard, but spin-down breaks long WS | 30–60 s after idle | Small | Low |
| **Railway** | $5 trial credit | $5/mo Hobby | Standard | None | Small | Low |
| **Heroku** | Eco $5/mo | $7/mo Basic | Standard | None on paid | Small | Low |
| **AWS API Gateway WebSocket + Lambda** | Pay-per-request | $0 at low usage, scales with traffic | Awkward (request/response model) | Lambda cold-start per msg | High (IAM, CF, CW) | High |
| **Azure Web PubSub** | 20 k free messages/day | $30/mo Standard_S1 | Native | None | Small | High |
| **Supabase Realtime** | Free up to 200 concurrent | $25/mo Pro | Native (Phoenix channels) | None | Small | High |
| **Hetzner / DigitalOcean / OVH VPS** | No | $4–5/mo CX11 / s-1vcpu-512mb | Standard | None | **High** (OS updates, TLS, monitoring) | None |
| **Personal computer + Cloudflare Tunnel** | Yes | $0 | Standard | None | Medium (always-on machine, electricity, your router) | None (the tunnel is removable) |

### Why Cloudflare Workers + Durable Objects is the default

- **Free tier is actually adequate** for the forecast usage. Workers Paid at
  $5/mo flat gives you 10M Worker requests / month included; Durable Object
  requests and duration are billed separately but trivially cheap for our
  traffic shape (short messages, few concurrent peers). The review doc sizes
  this.
- **Durable Objects pin one object instance per party id.** The DO runtime
  automatically routes `env.PARTY_ROOM.idFromName(partyId)` to the same worker
  host as long as the object is alive. No partitioning logic on our side.
- **WebSocket Hibernation API** lets a DO doze off between messages without
  dropping the connection, cutting charged CPU time. Not used in the initial
  plan (Phase 3) — easy win for a later optimization pass.
- **No cold start.** DOs stay warm per-party for the duration of the party.
- **Zero ops.** No container to patch, no OS to update, no TLS cert to rotate.

### Why not PartyKit

PartyKit is the friendliest-API option and is built for exactly this shape of
workload (rooms with broadcast). **If you don't already have Cloudflare opinions,
it's the faster path.** The reason the plan doesn't default to it:

- Its SDK would become a dependency of both server and client. Today both are
  5 KB of hand-written code; PartyKit would roughly double that footprint in
  deps for negligible feature gain.
- Acquired by Cloudflare in 2024 — pricing/roadmap still settling. A second
  vendor abstraction layer whose long-term position is uncertain.

Fine choice if you'd rather delete 60 lines of DO code. Plan is drop-in
compatible: the RelayClient doesn't know what host it's talking to.

### Why not Fly.io

Fly.io is the best **non-Cloudflare** option. Plain Docker, runs Node
unmodified, $2/mo for the smallest instance. The reason Cloudflare wins for
this workload:

- Fly.io needs a Dockerfile, a fly.toml, and `flyctl deploy`. About 40 lines of
  infrastructure code.
- Fly.io charges by instance-hour even when idle, Cloudflare charges by
  invocation. At typical party volume (one party of 5 active for 1 hour/day)
  Fly.io costs ~$2/mo, Cloudflare costs ~$0.
- Cloudflare's DO model exactly matches "one object per party id"; on Fly.io
  we'd implement routing ourselves (or accept all messages hit the same
  instance, which is fine for this scale).

**Fly.io becomes the better choice** if you need:
- Bring-your-own crypto / runtime that Workers can't host (e.g. a custom C
  binary)
- Multi-region with regional pinning
- You genuinely don't want any vendor abstraction, just a Linux VM

### Why not Render.com (free) — the trap

Render's free tier sounds perfect until you read the fine print: web services
on the free plan **spin down after 15 minutes of inactivity** and take 30–60
seconds to wake up. WebSocket connections during sleep are dropped. In practice
this means a party with a quiet 15-minute period (a loading screen, a boss
cutscene) would have every peer's connection severed.

Render's paid Starter ($7/mo) doesn't sleep, but at that price Fly.io is
cheaper and Cloudflare is free.

### Why not AWS / Azure / GCP managed services

Any of them can host this. None are cheaper or simpler than Cloudflare for this
workload. AWS API Gateway WebSockets in particular have a request/response
model (each message is a Lambda invocation, cold-start per message) that makes
party traffic awkward. If you're already in a given cloud for unrelated
reasons, it's fine to use that cloud. If you're choosing fresh — Cloudflare.

### Why the personal computer option exists

- Complete independence from any SaaS / vendor.
- Zero monetary cost if you already have an always-on PC.
- Full inspectability: `journalctl -f` shows every frame.

Tradeoffs in exchange:
- You are the uptime SLA. If your PC sleeps, reboots for Windows Update, or
  you go on vacation with it off, the party relay is down.
- Your home ISP's upstream bandwidth, latency, and NAT rules become the bottleneck
  and the firewall.
- A tunnel (Cloudflare Tunnel recommended) is still required because home ISPs
  routinely block inbound 80/443 and rotate dynamic IPs. See Task 26.

Phase 9 makes this a first-class supported path. Client code is unchanged; only
the URL in `config.json` differs.
