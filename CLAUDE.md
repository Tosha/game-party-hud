...
Windows-only WPF tray app that displays a party HUD with live HP bars read from the screen. See `docs/requirements.md` for the product overview and `docs/superpowers/specs/2026-04-16-game-party-hud-design.md` for the technical design.

## Hard constraints

1. **Windows-only.** Target framework is `net8.0-windows10.0.19041.0`. Do not add cross-platform abstractions.
2. **Free-tier hosting only.** The message relay is hosted in a separate repo on a free tier; no paid services. If you think we need a paid tier or a long-term background service, stop and raise the design question first.
